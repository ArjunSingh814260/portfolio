...
export default function Experience() {
  ...
  return (
    <section className="bg-[#121212] py-12 md:py-24 px-4 sm:px-6 lg:px-12 relative z-20 w-full">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Timeline Container */}
        <div ref={containerRef} className="relative md:ml-6 ml-4 select-none pb-10">
          ...
          <motion.div 
            ...
            className="absolute left-[-1px] -translate-x-1/2 top-0 bottom-0 w-[2px] bg-gradient-to-b from-blue-500 via-purple-500 to-transparent z-10" 
          />
          ...
          {experiences.map((exp, idx) => (
            <motion.div 
              ...
              className="mb-12 md:mb-24 pl-6 md:pl-16 relative group"
            >
              {/* Timeline Glowing Dot */}
              <div className="absolute left-[-1px] -translate-x-1/2 top-3 w-[14px] h-[14px] rounded-full bg-[#121212] border-[2px] border-blue-500 group-hover:border-purple-500 group-hover:bg-purple-500 group-hover:scale-150 transition-all duration-500 shadow-[0_0_15px_rgba(59,130,246,0.6)] group-hover:shadow-[0_0_25px_rgba(168,85,247,1)] z-20" />
              

              {/* Title & Period */}
              <div className="flex flex-col md:flex-row md:items-baseline md:justify-between mb-4">
                <h3 className="text-2xl md:text-5xl font-semibold text-white tracking-tight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-white/60 transition-all duration-300">
                  {exp.title}
                </h3>
                <span className="text-zinc-500 text-xs md:text-base font-medium tracking-[0.2em] uppercase mt-2 md:mt-0 flex-shrink-0">
                  {exp.period}
                </span>
              </div>
              
              {/* Company */}
              <div className="text-lg md:text-2xl text-blue-400 font-medium mb-4">
                {exp.company}
              </div>
              
              {/* Description */}
              <p className="text-zinc-400 text-base md:text-xl leading-relaxed max-w-3xl mb-8 group-hover:text-zinc-300 transition-colors duration-300">
                {exp.description}
              </p>
              ...
              <div className="flex flex-wrap gap-3">
                {exp.techStack.map((tech, i) => (
                  <motion.span 
                    ...
                    className="px-3 py-1.5 md:px-5 md:py-2 rounded-full border border-white/10 bg-white/[0.02] text-zinc-300 text-xs md:text-base backdrop-blur-md group-hover:bg-white/[0.08] group-hover:border-white/20 transition-all duration-300 hover:shadow-[0_4px_15px_rgba(0,0,0,0.5)] cursor-pointer"
                  >
                    {tech}
                  </motion.span>
                ))}
